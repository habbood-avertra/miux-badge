...
export function findDifference(expected: ElementStructure | ElementStructure[], actual: ElementStructure | ElementStructure[], strict: boolean): string
{
    ...
    {
        if(isJasmineAny(structure))
        {
            return structure.jasmineToString();
        }
        if(typeof structure === "string" || structure == null)
        {
            return `text '${structure}'`;
        }
        if(Array.isArray(structure))
        {
            return "[" + structure.map(toString).join(", ") + "]";
        }
...
export function toElementStructure(node: any): ElementStructure | ElementStructure[]
{
    ...
    {
        node = node.nodes.length > 1 ? node.nodes : node.nodes[0];
    }

    if(Array.isArray(node))
    {
        return node.map(n => toElementStructure(n) as ElementStructure);
    }
    if(node instanceof HTMLElement)
    {
        return domToStructure(node);
    }
    ...
    function reactToStructure(child: ReactChild | jasmine.Any): ElementStructure
    {
        ...
        if(isJasmineAny(child))
        {
            return child;
        } // todo: disallow Any inside createElement
        if(isValidElement(child))
        {
            return elemToStructure(child);
        }
